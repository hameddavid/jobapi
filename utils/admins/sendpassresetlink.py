...
def SendLink(payload: SendPassResetLink, db: Session ):  
   try:   
       ...
       sendPasswordResetLinkToAdmin(vToken, payload)
       raise HTTPException(status_code=200, detail=f"Admin.sendPassResetLink: Password reset link sent to {emailAddy}.")      
   except Exception as e: 
        db.rollback()        
        raise e  
def sendPasswordResetLinkToAdmin(ten_digit:str,payload:SendPassResetLink):        
      ...
      retMsg =  sendEmail(processor = payload.processor, subject = "Password Reset: JobPosting", 
